/*************
 * This is a copy of an older version of the moment typings
 * The current typings are near impossible to use on a non "import/export" style project 
 * While some workarounds got the code to compile, moment was still a typeless "global" variable
 * If a better workaround is found it will be put here.
 * Until then, know these definitions may become out of date over time -- feel free to edit to match the api if necessary
 *************/
...
interface Zone {
    name: string; // the unique identifier
    abbrs: string[]; // the abbreviations
    untils: number[]; // the timestamps in milliseconds
    offsets: number[]; // the offsets in minutes

    parse(timestamp: Date): number;
    offset(timestamp: number): number;
    abbr(timestamp: number): number;
}
...
/**
 * Contents of https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/moment-duration-format/index.d.ts (between 'declare module' brackets), minus the namespace
 */
